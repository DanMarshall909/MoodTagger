...
public class TagManager
{
    ...
    public TagManager(AppConfig config) => _config = config ?? throw new ArgumentNullException(nameof(config));
    ...
    public async Task<bool> WriteTagsAsync(MoodAnalysis analysis, CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(analysis.FilePath)) throw new FileNotFoundException($"File not found: {analysis.FilePath}");
        ...
        if (_config.CreateBackups) await CreateBackupAsync(analysis.FilePath, cancellationToken);
    ...
    private bool WriteTagsInternal(MoodAnalysis analysis)
    {
        try
        {
            ...
            if (analysis.Tempo > 0) tag.BeatsPerMinute = (uint)Math.Round(analysis.Tempo);
    ...
    private void WriteCustomTag(Tag tag, string description, string value)
    {
    ...
    private MoodAnalysis? ReadTagsInternal(string filePath)
    {
        try
        {
            ...
            var tag = file.GetTag(TagTypes.Id3v2) as Tag;
            ...
            // Read BPM
            if (tag.BeatsPerMinute > 0) analysis.Tempo = tag.BeatsPerMinute;
    ...
    private float ReadCustomTagFloat(Tag tag, string description)
    {
        string value = ReadCustomTagString(tag, description);

        if (!string.IsNullOrEmpty(value) && float.TryParse(value, out float result)) return result;
    ...
    private string ReadCustomTagString(Tag tag, string description)
    {
        foreach (var frame in tag.GetFrames())
            if (frame is UserTextInformationFrame textFrame &&
                textFrame.Description == description &&
