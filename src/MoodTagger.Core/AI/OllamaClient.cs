...
namespace MoodTagger.Core.AI
{
    ...
    public class OllamaClient
    {
        ...
        public async Task<MoodAnalysis> AnalyzeMoodAsync(AudioFeatures features, CancellationToken cancellationToken = default)
        {
            if (features == null)
            {
            ...
            return ParseResponse(response, features.FilePath);
        }
        ...
        private string GeneratePrompt(AudioFeatures features)
        {
            ...
            sb.AppendLine("You are an expert music analyzer. Analyze the following audio features and rate them according to the specified scales:");
            sb.AppendLine();
            ...
            sb.AppendLine("3. Groove Tightness (-5 to +5): -5 = highly swung/broken, 0 = straight, +5 = extremely tight quantized");
            sb.AppendLine("4. Funk/Swing (0 to 10): Groove funkiness. More swing, syncopation = higher score");
            sb.AppendLine("5. Dancefloor Use (1 to 5): 1 = ambient opener, 3 = peak groover, 5 = main drop bomb");
            sb.AppendLine("6. Layering Potential (0 to 10): How well it layers over others; e.g., tool tracks score high");
            sb.AppendLine("7. Tension (-5 to +5): -5 = deeply relaxing, +5 = anxiety-inducing / suspenseful");
            sb.AppendLine("8. Rhythmic Complexity (0 to 10): Polyrhythms, syncopation, odd time = high score");
            sb.AppendLine("9. Sound Palette (-5 to +5): -5 = organic/acoustic, +5 = synthetic/futuristic");
            sb.AppendLine();
            ...
            sb.AppendLine();
            sb.AppendLine("The track is: " + System.IO.Path.GetFileName(features.FilePath));
            
            if (features.Metadata.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine("Additional track metadata:");
                foreach (var meta in features.Metadata)
                {
        ...
        private MoodAnalysis ParseResponse(string response, string filePath)
        {
            ...
            // Use the detected BPM if available, otherwise try to extract from response
            if (ExtractRating(response, @"Tempo:\s*(\d+\.?\d*)", out float tempo))
            {
        ...
        private void ExtractExplanation(string response, string dimension, Dictionary<string, string> explanations)
        {
            ...
            if (match.Success && match.Groups.Count > 1)
            {
                explanations[dimension] = match.Groups[1].Value.Trim();
            }
        }
    }
}
